...
- Usando el vector de ingresos y gastos, ¿cuanto es el total de ingreso disponible en los 5 meses (Enero-Mayo)?
...
- Su compañero de clases quiere crear una secuencia de números de **cinco en cinco** hasta el 100. ¿Cómo lo puede hacer en `R`?
...
## Vector lógico y operadores {.scrollable .smaller}
...
## Indexación y segmentación 🍿 {.scrollable .smaller}
...
- Ahora su amigo quiere que lo ayude con este vector de nombres `c("María", "Pedro", "Juan")`. 
- Él necesita que cada nombre se repita tres veces en el orden que están cada nombre pero que además se repita dos veces todo el vector de nombres, tal que obtenga esto : `"María", "María", "María", "Pedro", "Pedro", "Pedro", "etc"`. ¿Cómo lo puede hacer en `R`?

## Solución ✅

```{r}
nombres <- c("María", "Pedro", "Juan")
rep(nombres, each = 3, times = 2)
```

## Veamos lo que has aprendido 🧠

1. Usted tiene la temperatura promedio de cada mes de un año `temp <- c(22.52, 18.70, 19.61, 22.79, 29.38, 30.19, 33.16, 36.97, 33.29, 28.98, 24.31, 22.43)`
- Cree un vector con los nombre de los meses y asigne los nombres al vector `temp`
- ¿Meses con la mayor y menor temperatura?
- ¿Meses con temperaturas mayores que 30 grados? 
- ¿Meses con temperaturas entre 35 y  40 grados? 
...
## Manipulación de listas `r emoji::emoji("book")`
